...
st.markdown("Top 500 validators ranked by profitability (net daily SOL after subtracting ~1.1 SOL/day voting costs). Most validators are underwater.")

dist_query = """
    SELECT net_daily_sol_after_votes,
           ROW_NUMBER() OVER (ORDER BY net_daily_sol_after_votes DESC) as validator_rank
    FROM `zoomcamp-test-2.solana_validator_economics.fct_validator_profitability`
    WHERE MOD(ABS(FARM_FINGERPRINT(pubkey)), 100) = 0
...
fig.add_hrect(y0=0, y1=50, fillcolor="#1D9E75", opacity=0.08, line_width=0)
fig.add_hrect(y0=-2, y1=0, fillcolor="#E24B4A", opacity=0.08, line_width=0)
fig.update_yaxes(range=[-2, 50], title="Net daily SOL — 50+")
fig.update_xaxes(range=[0, 500], title="Validators (ranked best to worst) — 500+")
...
st.markdown(f"""
- **The vast majority of validators are underwater** — they don't earn enough SOL to cover the 1.1 SOL/day voting cost.
- **The Nakamoto coefficient is in the concerning range** (~15-19), meaning only 15-19 validators would need to collude to control a third of the network.
- **Reward concentration is high** — the top 100 validators capture over 70% of all rewards.
- **At current SOL price (${current_sol_price:,.2f})**, voting alone costs ~${1.1 * current_sol_price:,.0f}/day before hardware expenses.
